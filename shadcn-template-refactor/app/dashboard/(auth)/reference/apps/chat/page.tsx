...
import { ChatSidebar, ChatContent } from "@/app/dashboard/(auth)/reference/apps/chat/components";
...
async function getChats() {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/dashboard/(auth)/reference/apps/chat/data/chats.json")
  );
...
async function getChatUser(id: number) {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/dashboard/(auth)/reference/apps/chat/data/contacts.json")
  );
