...
## 15.12 NOTIFICATION PREFERENCES

Backend supports full per-type per-channel notification preferences.
See **Part-26 §26.7** for detailed UI specification.

### Backend Endpoints
- `GET /api/v1/notifications/preferences` — Get current user preferences
- `PATCH /api/v1/notifications/preferences` — Update preferences

### UI
- Grid with rows = NotificationType (13 types), columns = NotificationChannel (IN_APP, EMAIL, SMS, PUSH, WEBHOOK)
- Toggle switches per cell
- Auto-save on toggle (debounced PATCH)
- Group by category: Listings, Interactions, Reviews, Subscriptions, Payments, Vendors, System

---

## 15.13 MODULE API SURFACE (BACKEND-ALIGNED)

Notifications module should provide:
- `useNotificationsList(params)` — GET /notifications
- `useUnreadCount()` — GET /notifications/unread-count
- `useMarkAsRead(notificationId)` — PATCH /notifications/:id/read
- `useMarkAllAsRead()` — POST /notifications/mark-all-read
