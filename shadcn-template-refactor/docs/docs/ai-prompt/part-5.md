...
### B) Portal Layouts (`app/dashboard/(auth)/layout.tsx` - shared for all portals)
- enforce guards (from WEB PART 4)
- provide navigation shell (sidebar/topbar) with portal detection
- provide portal-level context (portal name, tenant context)
- mount query providers if needed

Note: All portals share the same layout at `app/dashboard/(auth)/layout.tsx`.
Portal detection is handled by `NavMain` component based on pathname.
