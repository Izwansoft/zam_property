...
## 14.10 MODULE API SURFACE (BACKEND-ALIGNED)

See **Part-26** for full endpoint details, UI specs, and hook definitions.
...
- `useAuditActionTypes()` — GET /audit/action-types (for filter dropdowns)
- `useAuditTargetTypes()` — GET /audit/target-types (for filter dropdowns)
...
- `useCheckFeatureFlag(key)` — GET /feature-flags/check?key=... (any role)
