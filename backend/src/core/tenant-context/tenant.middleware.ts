...
export class TenantMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    const { identifier, source } = this.pickTenantIdentifier(headerTenantId, domain);

    const tenant = await this.resolveTenant(identifier, source, domain);
  ...
  private pickTenantIdentifier(
    ...
    throw new BadRequestException(
      'Tenant identifier missing. Provide X-Tenant-ID header or use a tenant subdomain.',
    );
  }
